...
/*
TODO: Score should be probably be a Double from 0.0 to 1.0 instead of Boolean
 */
...
/**
  ...
  */
trait MonotonicValue extends IndexedConstraint {
...
//    for(index <- lo to hi) yield {
//      val t = ConstraintsUtil.compare(p.songs(index), af, f)
//      Score(t._1, Some(Info(af, index, t._2)))
//    }
//  }
}
...
/**
  ...
  */
trait MonotonicTransition extends IndexedConstraint {
  ...
  def score(p: Playlist, f: (Double, Double) => Boolean) = {
    ...
    for (index <- lo until hi) yield {
      (for {
      x <- p.songs(index).features.map(af.extract)
      y <- p.songs(index + 1).features.map(af.extract)
      } yield {
        val distance = ConstraintsUtil.monotonicDistance(x, y, af)
        Score(
          f(x, y),
          Some(Info(af, index, distance))
        )
      }).getOrElse(Score(matched = false, Some(Info(af, index))))
    }
  }
}
//
...
/**
  ...
  * @param lo the lower bound index of the song in the playlist
  * @param hi the upper bound index of the song in the playlist
  * @param af the AudioAttribute on which each Song in the range is evaluated on
  */
case class IncreasingTransition(lo: Int, hi: Int, af: AudioFeature) extends MonotonicTransition {
...
/**
  ...
  * @param that the AudioAttribute on which each Song in the range is evaluated on
  */
case class DecreasingTransition(lo: Int, hi: Int, af: AudioFeature) extends MonotonicTransition {
  override def score(p: Playlist): Seq[Score] = score(p, (x, y) => x > y)
}
